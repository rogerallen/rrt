...
NVCC_FLAGS     = -ccbin $(HOST_COMPILER) --compiler-options "$(GCC_WARNINGS)" -m64 $(NVCC_DBG) $(NVCC_GENCODE)
...
rrtc: $(SRCSC) $(INCS)
	$(HOST_COMPILER) $(HOST_REL) $(GCC_WARNINGS) -DUSE_FLOAT $(SRCSC) -o $@

# 3) CPU with OpenMP & double-precision
rrto: $(SRCSC) $(INCS)
	$(HOST_COMPILER) $(HOST_REL) $(GCC_WARNINGS) -fopenmp $(SRCSC) -o $@ -lgomp
